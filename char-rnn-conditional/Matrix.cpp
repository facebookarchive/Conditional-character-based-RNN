...
void Matrix::readMatrix(std::ifstream& file) {
  delete[] data_;
  char* memblock;
  double* datas;

  memblock = new char[2 * sizeof(int)];
  file.read(memblock, 2 * sizeof(int));
  m_ = *((int*)memblock);
  n_ = *(((int*)memblock) + 1);
  delete[] memblock;
  ...
  memblock = new char[m_ * n_ * sizeof(double)];
  file.read(memblock, m_ * n_ * sizeof(double));
  datas = (double*) memblock;
